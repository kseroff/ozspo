(function ($, Drupal, drupalSettings) {
    Drupal.behaviors.notes = {
      attach: function (context, settings) {
        ...
        $('.note-editable', context).once('notes').on('click', function () {
          $(this).prop('readonly', false); // Включить редактирование поля
        }).on('blur', function () {
          $(this).prop('readonly', true); // Отключить редактирование поля при потере фокуса
        });
        ...
        $('input.form-submit', context).once('notes').each(function () {
          ...
          $('.delete-note', context).once('notes').on('click', function () {
            var noteId = $(this).data('note-id');
            var $noteField = $('#note_' + noteId);
            var ajaxSettings = {
